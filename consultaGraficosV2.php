...
<body>
  ...
  <div class="container-fluid page-container" style="margin: 0px;">
      <div class="no">
        ...
        $sql = "SELECT distinct id_sensor FROM `sensors` where id_sensor ".$a." order by id_sensor;";  
    ...
    window.onload = SeeSensor('<?php echo $a ?>');

    function resetZoom(){
      window.location.reload();
    }
    
    function getSensorIDsFromURL() {
        var urlParams = new URLSearchParams(window.location.search);
        var sensorIDs = urlParams.get('sensor');
        // Remova os caracteres de parênteses e espaços extras
        sensorIDs = sensorIDs.replace(/[\(\)]/g, '').split(',');
        return sensorIDs;
    }
