              ...
              <div style="margin: 0 auto; text-align: center; width:40vh; overflow:auto ; height: 60%;">
               ...
               if ($mysqli->connect_errno) {
                ...
                <table width='100%' style="overflow:auto, max-height:500px" >
                  <td style="width:2px;" ><button disabled style="margin-bottom: 500px;height: 65px;width: 49px;padding-right: 8px;border-right-width: 0px;" class="btn btn-secondary d-flex flex-column justify-content-center" ><span class="rotate">No</span></button></td>
                    <td valign="top" style="width:2px;" ><?php 
                    //$sql = "SELECT distinct id_sensor,CONV(RIGHT(id_sensor,2),16,10) as valor_dec FROM `location` where status = 1 order by id_sensor limit ".round($total/3).";";  
                    $sql= "SELECT id_sensor, RIGHT(id_sensor,2) as valor_dec from location where status = 1 limit ".round($total/3).";";
                    $result = $mysqli->query($sql);
                    while($row = mysqli_fetch_array($result)){
                      echo '<button style="margin-bottom: 10px;width:73px;" class="btn btn-md btn-secondary btn-sm btn-block buttonSensor" onClick="SeeSensor(this.id)" id='. $row["id_sensor"].'  > '. $row["valor_dec"].'</button>';
                    }
                    ?>
                    
                  </td>
                    <td valign="top" style="width:-20px; width:2px;"><?php
                    //$sql = "SELECT distinct id_sensor,CONV(RIGHT(id_sensor,2),16,10) as valor_dec FROM `location` where status = 1 order by id_sensor limit ".round($total/3)." offset ".round($total/3).";";  
                    $sql= "SELECT id_sensor, RIGHT(id_sensor,2) as valor_dec from location where status = 1 limit ".round($total/3)." offset ".round($total/3).";";
                    $result = $mysqli->query($sql);
                    while($row = mysqli_fetch_array($result)){
                      echo '<button style="margin-bottom: 10px; width:73px;" class="btn btn-md btn-secondary btn-sm btn-block buttonSensor" onClick="SeeSensor(this.id)" id='. $row["id_sensor"].'  > '. $row["valor_dec"].'</button>';
                    }
                    ...
                    <td valign="top"style="width:-20px;" ><?php
                    //$sql = "SELECT distinct id_sensor,CONV(RIGHT(id_sensor,2),16,10) as valor_dec FROM `location` where status = 1 order by id_sensor limit 100 offset ".(round($total/3)*2).";";  
                    $sql= "SELECT id_sensor, RIGHT(id_sensor,2) as valor_dec from location where status = 1 limit 100 offset ".(round($total/3)*2).";";
                    $result = $mysqli->query($sql);
                    while($row = mysqli_fetch_array($result)){ 
                      echo '<button style="margin-bottom: 10px; width:73px;" class="btn btn-md btn-secondary btn-sm btn-block buttonSensor" onClick="SeeSensor(this.id)" id='. $row["id_sensor"].'  > '. $row["valor_dec"].'</button>';
                    }
